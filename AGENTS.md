# go-boot-kitex 项目开发规范文档

go-boot-kitex 是一个基于 [github.com/xudefa/go-boot](https://github.com/xudefa/go-boot) 的 Kitex RPC 框架集成模块。本模块将 Kitex 框架无缝集成到 go-boot 的 IoC 容器和自动配置体系中,遵循 go-boot 项目的开发规范。
...
- **集成模块**:go-boot-kitex 是 go-boot 的 RPC 层集成,将 Kitex 作为 RPC Server/Client 实现
- **规范继承**:完全遵循 go-boot 的开发规范、命名约定、代码风格

### 1.2 核心职责

- 将 Kitex Server/Client 注册为 go-boot 容器中的 Bean
- 提供基于 go-boot 自动配置的 Kitex 启动器
- 支持服务注册、中间件配置的声明式配置
- 内置中间件支持(日志、恢复等)
...
- **RPC 框架**:集成 Kitex(`github.com/cloudwego/kitex`)
- **职责边界**:仅负责 Kitex Server/Client 集成,不包含业务逻辑
- **示例代码**:统一放在 `examples/` 目录,演示 Kitex 集成用法

### 2.2 go-boot-kitex 核心包结构

| 包 | 说明 | 核心类型/函数 |
|---|------|----------|
| `kitex/` | Kitex 自动配置 | `KitexAutoConfiguration` |
| `server/` | Kitex Server 集成 | `Server`, `New`, `WithAddress`, `RegisterService` |
| `client/` | Kitex Client 集成 | `Client`, `New`, `WithAddress`, `WithTimeout`, `WithRetry` |
| `middleware/` | Kitex 中间件 | 日志、恢复中间件 |

### 2.3 go-boot 核心包参考

go-boot-kitex 依赖 go-boot 的以下核心包:
...
go-boot-kitex 遵循 go-boot 的接口抽象原则,所有集成层通过核心框架中的接口抽象定义,实现运行时互换:
...
func init() {
    boot.RegisterAutoConfig(
        &KitexAutoConfiguration{},
        condition.OnProperty("kitex.server.enabled", "true"),
    )
}
```
